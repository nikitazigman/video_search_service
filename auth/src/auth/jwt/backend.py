...
class JWTAuthorizationBackend:
    ...
    async def generate_jwt_credentials(
        ...
        return jwt_schemas.AccessToken(
    ...
    async def revoke_jwt_credentials(
        ...
        await self.jwt_storage.revoke_token(
            ...
            expire_secs=settings.verify_access_token.access_token_expires_secs,
        )
        await self.jwt_storage.revoke_token(
            jti=decoded_token.refresh_jti,
            expire_secs=settings.verify_access_token.refresh_token_expires_secs,
        )
        return None
